...
fn main() {
    ...
    let input_row_count = input.len();
    let input_col_count = 140;

    let mut workable_array: [[char; 140]; 140] = [['.'; 140]; 140];

    for i in 0..input_row_count {
        let row = input[i].as_bytes();
        for j in 0..input_col_count {
            workable_array[i][j] = row[j] as char;
        }
    }

    let mut tracking_number = false;
    let mut tracking_chars: [char;140] = ['0';140];
    let mut tracking_count = 0;

    let mut symbol_indexes: HashSet<(usize,usize)> = HashSet::new(); // put every 

    for (row_index,row) in workable_array.iter().enumerate() {
        if row_index-2 >= 0 { // ignore the first row since we need to loop over the 2nd one to get all the symbols first
            // Solve 2 rows above since we have all the information

        }
        for (col_index,entry) in row.iter().enumerate() {
            if entry.is_digit(10) {
                tracking_number = true;
                tracking_chars[tracking_count] = *entry;
                tracking_count += 1;
            }
            else {
                match entry { 
                    '.' => {
                        ...
                        // we where tracking a number but now we found a dot, lets see if it is connected to any symbol.
                        tracking_count = 0;
                    }
                    _ => {
